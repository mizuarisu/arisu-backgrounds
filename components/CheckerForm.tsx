...
function StatusBadge({ children, variant = 'default' }: { children: React.ReactNode; variant?: 'default' | 'success' | 'danger' | 'warning' | 'info' }) {
  const styles = {
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex gap-4 py-3 text-sm" style={{ borderBottom: '1px solid var(--border)' }}>
      <span className="w-44 shrink-0" style={{ color: 'var(--muted)' }}>{label}</span>
...
function Avatar({ src, name, size = 40, flagged = false }: { src: string | null; name: string; size?: number; flagged?: boolean }) {
  ...
  if (src && !errored) {
    return (
      <img
        ...
        width={size}
        height={size}
        className={`rounded-full object-cover ${flagged ? 'ring-2 ring-red-500' : ''}`}
        style={!flagged ? { border: '1px solid var(--border)', width: size, height: size } : { width: size, height: size }}
  ...
  return (
    <div
      className={`rounded-full flex items-center justify-center text-xs font-mono shrink-0 ${flagged ? 'ring-2 ring-red-500' : ''}`}
      style={{ width: size, height: size, background: 'var(--muted-bg)', color: 'var(--muted)', border: '1px solid var(--border)', fontSize: size > 40 ? 16 : 11 }}
    >
      {name[0]?.toUpperCase()}
    </div>
  )
...
export default function CheckerForm() {
  ...
  const riskVariant: 'danger' | 'warning' | 'success' = riskScore >= 4 ? 'danger' : riskScore >= 1 ? 'warning' : 'success'
  ...
  const badgeChartData = data
    ? Object.entries(data.badges.byYear)
        .sort(([a], [b]) => Number(a) - Number(b))
        .map(([year, count]) => ({ year, badges: count }))
    : []
  ...
  const exportChartAsImage = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(chartRef.current, { backgroundColor: bgColor, scale: 2, useCORS: true })
  ...
  const conclusion = data ? [
    ...
    `Overall risk: ${risk}`,
    riskScore >= 4
      ? 'Recommendation: Exercise extreme caution. Multiple flagged associations.'
      : riskScore >= 1
      ? 'Recommendation: Review flagged associations before proceeding.'
    ...
    <div>
      ...
      <div className="flex gap-2 mb-6">
        ...
        <button
          ...
          className="px-5 py-3 text-sm font-medium disabled:opacity-40"
          style={{ background: 'var(--foreground)', color: 'var(--background)', borderRadius: 4 }}
        >
          {loading ? 'Checking...' : 'Check'}
        </button>
        {data && (
          <button onClick={clear} className="px-4 py-3 text-sm" style={{ border: '1px solid var(--border)', borderRadius: 4, color: 'var(--muted)' }}>
            Clear
          </button>
        )}
      </div>
      ...
      {error && (
        <div className="mb-6 px-4 py-3 text-sm rounded" style={{ background: 'var(--danger-bg)', color: 'var(--danger)', border: '1px solid var(--danger)' }}>
          {error}
        </div>
      )}
        ...
        <div>
              ...
              {flaggedFriends.map(f => {
                ...
                return (
                  <div key={f.id} className="flex items-start gap-3 px-4 py-3 rounded text-sm" style={{ background: 'var(--warning-bg)', border: '1px solid var(--warning)', color: 'var(--warning)' }}>
                    <span>⚠ <strong>Flagged friend:</strong> {f.name}{bl?.reason ? ` — ${bl.reason}` : ''}{' '}
                      <a href={`https://www.roblox.com/users/${f.id}/profile`} target="_blank" rel="noopener noreferrer" className="underline">View ↗</a>
                    </span>
                  </div>
                )
              })}
              {flaggedGroups.map(g => {
                const bl = blacklist.groups.find(bg => bg.value === String(g.group.id))
                return (
                  <div key={g.group.id} className="flex items-start gap-3 px-4 py-3 rounded text-sm" style={{ background: 'var(--danger-bg)', border: '1px solid var(--danger)', color: 'var(--danger)' }}>
                    <span>⚠ <strong>Flagged group:</strong> {g.group.name}{bl?.reason ? ` — ${bl.reason}` : ''}{' '}
                      <a href={`https://www.roblox.com/groups/${g.group.id}`} target="_blank" rel="noopener noreferrer" className="underline">View ↗</a>
                    </span>
                  </div>
                )
              })}
          ...
          <Section title="Profile">
            <div className="flex items-start gap-4 mb-4">
              {/* Profile avatar from thumbnails API */}
              <Avatar src={data.profileAvatarUrl} name={data.user.name} size={64} />
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-xl font-semibold">{data.profile?.displayName || data.user.name}</span>
                  <StatusBadge variant={riskVariant}>{risk}</StatusBadge>
                  {data.profile?.isBanned && <StatusBadge variant="danger">Banned</StatusBadge>}
                  {data.profile?.hasVerifiedBadge && <StatusBadge variant="info">Verified ✓</StatusBadge>}
                </div>
                <div className="text-sm mt-1" style={{ color: 'var(--muted)' }}>@{data.user.name} · ID: {data.user.id}</div>
                <a href={`https://www.roblox.com/users/${data.user.id}/profile`} target="_blank" rel="noopener noreferrer" className="text-xs underline mt-1 inline-block" style={{ color: 'var(--muted)' }}>
                  View on Roblox ↗
                </a>
              </div>
            </div>
          ...
          <Section title={`Group rank — ${TARGET_GROUP}`}>
            {targetGroup
              ? <Row label={targetGroup.group.name}><StatusBadge variant="info">{targetGroup.role.name}</StatusBadge><span className="text-xs font-mono" style={{ color: 'var(--muted)' }}>rank {targetGroup.role.rank}</span></Row>
              : <p className="text-sm" style={{ color: 'var(--muted)' }}>Not a member of group {TARGET_GROUP}.</p>}
          </Section>
              ...
              <div className="divide-y" style={{ borderColor: 'var(--border)' }}>
                {divisionGroups.map(g => (
                  <div key={g.group.id} className="flex items-center gap-3 py-3">
                    <a href={`https://www.roblox.com/groups/${g.group.id}`} target="_blank" rel="noopener noreferrer" className="text-sm flex-1 underline underline-offset-2" style={{ color: 'var(--foreground)' }}>{g.group.name}</a>
                    <StatusBadge>{g.role.name}</StatusBadge>
              ...
              : <>
                  <div className="flex flex-wrap gap-3">
                    {friendsToShow.map(f => {
                      ...
                      return (
                        <a key={f.id} href={`https://www.roblox.com/users/${f.id}/profile`} target="_blank" rel="noopener noreferrer" className="group relative" title={f.name}>
                          <Avatar src={f.thumbnailUrl} name={f.name} size={40} flagged={flagged} />
                  ...
                  {data.friends.length > 40 && !showAllFriends && (
                    <button onClick={() => setShowAllFriends(true)} className="text-xs mt-3 underline" style={{ color: 'var(--muted)' }}>
            ...
            {data.groups.length === 0
              ...
              : <>
                    ...
                    {groupsToShow.map(g => {
                      ...
                      return (
                        <div key={g.group.id} className={`flex items-center gap-3 py-3 ${flagged ? 'text-red-700 dark:text-red-400' : ''}`}>
                          ...
                          {isDivision && <StatusBadge variant="info">Division</StatusBadge>}
                          <a href={`https://www.roblox.com/groups/${g.group.id}`} target="_blank" rel="noopener noreferrer" className="text-sm flex-1 underline underline-offset-2" style={flagged ? {} : { color: 'var(--foreground)' }}>
                            {g.group.name}
                          </a>
                          <StatusBadge>{g.role.name}</StatusBadge>
                          <span className="text-xs font-mono" style={{ color: 'var(--muted)' }}>{g.group.id}</span>
                  ...
                  {data.groups.length > 25 && !showAllGroups && (
                    <button onClick={() => setShowAllGroups(true)} className="text-xs mt-3 underline" style={{ color: 'var(--muted)' }}>
          ...
          <Section title="Badges">
            <Row label="Total badges"><span className="font-mono text-sm">{data.badges.total}</span></Row>
            {badgeChartData.length > 0 && (
              <div className="mt-4">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-xs" style={{ color: 'var(--muted)' }}>Badges acquired by year</p>
                  <button onClick={exportChartAsImage} disabled={exportingChart} className="text-xs px-3 py-1 rounded disabled:opacity-50" style={{ border: '1px solid var(--border)', color: 'var(--muted)' }}>
                    {exportingChart ? 'Exporting...' : 'Export as PNG'}
                  </button>
                </div>
                <div ref={chartRef} style={{ background: 'var(--background)', padding: 16, borderRadius: 4, border: '1px solid var(--border)' }}>
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={badgeChartData}>
                      ...
                      <Bar dataKey="badges" fill="#3b82f6" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
          ...
          <Section title="Accessories & inventory">
            ...
            <Row label="Collectibles (visible)"><span className="font-mono text-sm">{data.collectibles}</span></Row>
          </Section>
